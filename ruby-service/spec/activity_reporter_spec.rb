  ...
  describe '#generate_report' do
    ...
    context 'when activities exist' do
      it 'returns a structured report hash with expected keys' do
        ...
        expect(report[:generated_at]).to eq('2024-01-03T12:00:00Z')
  ...
  describe '#format_timeline' do
    ...
    context 'when grouping by day' do
      it 'groups activities by day and sorts by period' do
        ...
        expect(result[0][:period]).to eq('2024-01-01')
        expect(result[0][:total_actions]).to eq(3)
        expect(result[0][:actions]).to eq('login' => 1, 'click' => 2)
        expect(result[0][:first_timestamp]).to eq('2024-01-01T10:15:00Z')
        expect(result[0][:last_timestamp]).to eq('2024-01-01T11:05:00Z')

        expect(result[1][:period]).to eq('2024-01-02')
  ...
  describe '#compare_users' do
    ...
    context 'when multiple users are provided' do
      ...
      it 'returns comparison data for all users' do
        ...
        expect(result[:comparisons].map { |c| c[:user_id] }).to match_array([1, 2, 3])
      end
  ...
  describe '#parse_timestamp' do
    ...
    context 'with an invalid timestamp string' do
      it 'returns Time.now as a fallback' do
        fixed_now = Time.parse('2024-01-03T12:00:00Z')
