...
RSpec.describe ActivityReporter do
  ...
  describe '#initialize' do
    it 'sets the go_service_url and python_service_url with provided values' do
      instance = described_class.new(
        go_service_url: 'http://custom-go:9000',
        python_service_url: 'http://custom-python:9001'
      )

      expect(instance.instance_variable_get(:@go_service_url)).to eq('http://custom-go:9000')
      expect(instance.instance_variable_get(:@python_service_url)).to eq('http://custom-python:9001')
    end
  ...
  describe '#compare_users' do
    ...
    let(:activities_user3) do
      [
        ...
        { 'timestamp' => '2024-01-02T11:00:00Z', 'action' => 'click' }
      ]
    end
    ...
    let(:stats_user3) do
      {
        total_actions: 3,
        unique_actions: 2,
        action_counts: { 'logout' => 1, 'click' => 2 },
    ...
    context 'when multiple users are provided' do
      it 'returns comparison data sorted by engagement_score descending' do
        ...
        expect(top[:user_id]).to eq(3)
        expect(top[:total_actions]).to eq(3)
        expect(top[:most_frequent_action]).to eq('click')

        expect(result[:top_user]).to eq(3)
      end

      it 'calculates the average engagement score with two decimal precision' do
        result = reporter.compare_users(user_ids)
