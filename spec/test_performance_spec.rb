...
RSpec.describe ReportGenerator do
  ...
  describe '#generate_user_report' do
    ...
    let(:user1) do
      double('User', name: 'Alice', posts: double('PostsAssoc1', count: 3))
    end

    let(:user2) do
      double('User', name: 'Bob', posts: double('PostsAssoc2', count: 5))
    end
    ...
    context 'when user has no posts association' do
      let(:user_without_posts) do
        double('User', name: 'Charlie', posts: double('PostsAssoc', count: 0))
      end
  ...
  describe '#build_csv' do
    ...
    context 'with records having nil attributes' do
      ...
      it 'converts nil attributes to empty string representation in CSV' do
        result = report_generator.build_csv(records)
        expect(result).to eq(",\n")
      end
    end
  ...
  describe '#find_matches' do
    ...
    context 'when one list is empty' do
      ...
      it 'returns an empty array when list_b is empty' do
        result = report_generator.find_matches(list_b, [])
        expect(result).to eq([])
      end
    end
    ...
    context 'when lists contain non-numeric elements' do
      ...
      it 'matches based on equality for strings' do
        result = report_generator.find_matches(list_a, list_b)
        expect(result).to match_array(['b', 'c'])
      end
    end

    context 'when one of the lists is nil' do
      let(:list_a) do
        [1, 2, 3]
      end
      ...
      it 'raises NoMethodError when list_a is nil' do
        expect do
          report_generator.find_matches(nil, [1, 2, 3])
        end.to raise_error(NoMethodError)
      end
    end
  end
